AnnieRoot.p3_2= AnnieRoot.p3_2||{};
AnnieRoot.p3_2.P3_2= function(){
	var s=this;
	annie.MovieClip.call(s);
	/*_a2x_need_start*/s.bt_1=null;s.bt_2=null;s.bt_3=null;s.bt_4=null;s.bt_5=null;s.bt_6=null;s.bt_back=null;s.bt_end=null;s.fb_1=null;s.fb_2=null;s.fb_3=null;s.mc=null;s.mc_1=null;s.mc_2=null;s.mc_3=null;s.mc_Q=null;s.mc_hua=null;/*_a2x_need_end*/
	annie.initRes(s,"p3_2","P3_2");
	var bt_list=[
		s.bt_1,
		s.bt_2,
		s.bt_3,
	...
	s.addEventListener(annie.MouseEvent.MOUSE_UP,function (e) {
		...
		switch (e.target.name) {
			...
			case 'fb_3':
				...
				s.gotoAndStop([3,4,5][num-1]);
				if(num>1){
					s.mc.gotoAndStop(num-1);
					show_hua();//显示滑动条
				}
				break
			case 'bt_back':
			...
			case 'bt_qie'://治疗/预防
				e.target.alpha=0;
				s.gotoAndStop(s.currentFrame==6?7:6);
				s.mc_1.gotoAndStop(1);
				s.mc_2.gotoAndStop(1);
				break
			case 'bt_prev':e.target.alpha=0;s.mc_1.prevFrame();s.mc_2.prevFrame();s.mc_3.prevFrame();break
			case 'bt_next':e.target.alpha=0;s.mc_1.nextFrame();s.mc_2.nextFrame();s.mc_3.nextFrame();break
		}
		//监测 视频播放
	...
	function show_hua(){//是否启用滑动条
		...
		setTimeout(function (e){
			...
			scroll_max=520-s.mc.getWH().h;//获取图片超出遮罩长度
			if(scroll_max<0){
	...
	function Check_video(){
		if([3].includes(s.currentFrame) && s.mc_Q.visible==false){
			videoPlay_1('p3_2');//默认播放视频
			//补丁，改变位置大小
			if(s.currentFrame==3){
				var rect={x:778, y:261, w:1040, h:588,}//视频位置宽高
				floatDisplay.scaleX = rect.w/videoPlayer.width;//旧版尺寸
				floatDisplay.scaleY = rect.h/videoPlayer.height;//旧版尺寸
				floatDisplay.x = rect.x;
				floatDisplay.y = rect.y;
...
annie.A2xExtend(AnnieRoot.p3_2.P3_2,annie.MovieClip);